...
local function deepclone(args: table, copies: table): table
    ...
    elseif typeof(args) == "Instance" then
        copy = cloneref(args)
    elseif type(args) == "buffer" then
        -- properly copy buffer byte by byte so snapshot is independent
        local len = buffer.len(args)
        local buf = buffer.create(len)
        buffer.copy(buf, 0, args, 0, len)
        copy = buf
    else
        copy = args
    end
...
local blocklist = {}
--- The args blacklist: entries can be a plain string (exact match) or {key=..., value=...} for table key-value matching (case-insensitive)
local argsblacklist = {}
...
--- Converts an escape string (e.g. "\027\000\000") to a buffer for comparison
...
--- Compares two buffers byte-by-byte, returns true if identical
local function buffersEqual(a, b)
    local la = buffer.len(a)
    local lb = buffer.len(b)
...
--- Recursively checks if args match any entry in argsblacklist.
--- Entry types:
---   "plainstring"             → exact case-insensitive match against any string/number/boolean in args
---   {key = "K"}               → match if any table in args has key K (case-insensitive)
---   {value = "V"}             → match if any table in args has value V (case-insensitive)
---   {key = "K", value = "V"}  → match if any table in args has key K with value V (both case-insensitive)
---   {buffer = "\027\000\000"} → match buffer arg using escape string (manual via getgenv)
---   {buffer = <raw buffer>}   → match buffer arg directly by byte content (from Exclude (s) snapshot)
local function checkArgsBlacklist(args, depth)
    depth = depth or 0
    if depth > 10 or #argsblacklist == 0 then return false end
    for _, v in next, args do
        local tv = type(v)
        if tv == "string" or tv == "number" or tv == "boolean" then
            local vs = lower(tostring(v))
            for _, pattern in next, argsblacklist do
                if type(pattern) == "string" and lower(pattern) == vs then
                    return true
                end
            end
        elseif tv == "buffer" then
            for _, pattern in next, argsblacklist do
                if type(pattern) == "table" then
                    local pb = pattern.buffer
                    if pb ~= nil then
                        if type(pb) == "buffer" then
                            -- direct buffer-to-buffer compare (from Exclude (s))
                            if buffersEqual(v, pb) then
                                return true
                            end
                        elseif type(pb) == "string" then
                            -- escape string pattern (from getgenv manual input)
                            local ok, patbuf = pcall(stringToBuffer, pb)
                            if ok and buffersEqual(v, patbuf) then
                                return true
                            end
                        end
                    end
        ...
        elseif tv == "table" then
            -- check key-value patterns against this table
            for _, pattern in next, argsblacklist do
                if type(pattern) == "table" then
                    local matchKey = pattern.key
                    local matchVal = pattern.value
                    if matchKey or matchVal then
                        for k, kv in next, v do
                            local ks = lower(tostring(k))
                            local kvs = lower(tostring(kv))
                            local keyOk = (not matchKey) or (ks == lower(tostring(matchKey)))
                            local valOk = (not matchVal) or (kvs == lower(tostring(matchVal)))
                            if keyOk and valOk then
                                return true
                            end
                        end
                    end
            ...
            if checkArgsBlacklist(v, depth + 1) then
                return true
            end
        end
    end
...
local newindex = function(method,originalfunction,...)
    if typeof(...) == 'Instance' then
        ...
        if remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") or remote:IsA("UnreliableRemoteEvent") then
            ...
            if not tablecheck(blacklist,remote,id) and not IsCyclicTable(args) and not checkArgsBlacklist(args) then
                local data = {
                    method = method,
                    remote = remote,
                    args = deepclone(args),
...
--- Excludes remotes by args value via getgenv().SimpleSpy_ArgsBlacklist.
--- Supported entry types:
---   "plainstring"              → exact case-insensitive match on any string/number/boolean arg
---   {key = "K"}               → match table arg that has key K
---   {value = "V"}             → match table arg that has value V
---   {key = "K", value = "V"}  → match table arg that has key K with value V
---   {buffer = "\027\000\000"} → match buffer arg with identical byte content
newButton(
    "Exclude (a)",
    function()
        return "Exclude remotes by args content via getgenv.\n" ..
            "Set getgenv().SimpleSpy_ArgsBlacklist = {entries} then click.\n" ..
            "String: {\"value\"}  |  Key-value: {{key=\"K\",value=\"V\"}}\n" ..
            "Buffer: {{buffer=\"\\027\\000\\000\"}}  |  Key only: {{key=\"K\"}}"
    end,
    function()
        local list = getgenv().SimpleSpy_ArgsBlacklist
        if type(list) ~= "table" then
            TextLabel.Text = "Set getgenv().SimpleSpy_ArgsBlacklist first!"
            return
        end
        local added = 0
        for _, entry in next, list do
            if type(entry) == "string" and entry ~= "" then
                -- plain string
                table.insert(argsblacklist, entry)
                added += 1
            elseif type(entry) == "table" then
                if entry.key or entry.value or type(entry.buffer) == "string" then
                    -- key-value or buffer entry
                    table.insert(argsblacklist, entry)
                    added += 1
                end
            end
        end
        getgenv().SimpleSpy_ArgsBlacklist = nil
        if added > 0 then
            TextLabel.Text = ("Args BL: added %d entry(s)!"):format(added)
        else
            TextLabel.Text = "No valid entries found in SimpleSpy_ArgsBlacklist!"
        end
    end
)

--- Clears the args blacklist
newButton(
    "Clr Args BL",
    function() return "Click to clear the args blacklist.\nRemotes previously excluded by args will be logged again." end,
    function()
        argsblacklist = {}
        TextLabel.Text = "Args blacklist cleared!"
    end
)

--- Excludes the currently selected remote based on its entire args snapshot.
--- Each arg value is added to argsblacklist so any future remote firing with
--- the same args (string/number/boolean/buffer/table key-values) will be ignored.
newButton(
    "Exclude (s)",
    function() return "Click to exclude remotes that match the selected remote's current args.\nAll arg values are snapshotted and added to the args blacklist." end,
    function()
        ...
        if not args or #args == 0 then
            TextLabel.Text = "Selected remote has no args!"
            return
        end

        local added = 0

        local function extractEntries(tbl, depth)
            if depth > 10 then return end
            for _, v in next, tbl do
                local tv = type(v)
                if tv == "string" and v ~= "" then
                    table.insert(argsblacklist, v)
                    added += 1
                elseif tv == "number" or tv == "boolean" then
                    table.insert(argsblacklist, tostring(v))
                    added += 1
                elseif tv == "buffer" then
                    -- store raw buffer object directly — no round-trip conversion needed
                    table.insert(argsblacklist, {buffer = v})
                    added += 1
                elseif tv == "table" then
                    -- snapshot key-value pairs from this table
                    for k, kv in next, v do
                        local tkey = type(k)
                        local tval = type(kv)
                        if (tkey == "string" or tkey == "number") and (tval == "string" or tval == "number" or tval == "boolean") then
                            table.insert(argsblacklist, {key = tostring(k), value = tostring(kv)})
                            added += 1
                        end
                    end
                    extractEntries(v, depth + 1)
                end
            end
        end

        extractEntries(args, 0)

        if added > 0 then
            TextLabel.Text = ("Args BL: snapshotted %d value(s) from selected!"):format(added)
        else
            TextLabel.Text = "No blacklistable values found in selected args!"
        end
    end
)
